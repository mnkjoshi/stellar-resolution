...
const mapKnowledge = {
	unwise: {
		...
		description: 'Wide-field Infrared Survey Explorer (WISE) astronomical survey data',
		coordinate_system: 'equatorial',
		bounds: { ra: { min: 0, max: 360 }, dec: { min: -90, max: 90 } },
		notable_objects: {
			'galactic center': { ra: 266.4, dec: -29.0, description: 'Center of the Milky Way galaxy' },
			'andromeda galaxy': { ra: 10.7, dec: 41.3, description: 'Nearest major galaxy to Milky Way' },
			'orion nebula': { ra: 83.8, dec: -5.4, description: 'Famous star-forming region' },
			vega: { ra: 279.2, dec: 38.8, description: 'Bright star in constellation Lyra' },
			betelgeuse: { ra: 88.8, dec: 7.4, description: 'Red supergiant star in Orion' },
			sirius: { ra: 101.3, dec: -16.7, description: 'Brightest star in the night sky' },
			pleiades: { ra: 56.8, dec: 24.1, description: 'Famous star cluster' },
			'crab nebula': { ra: 83.6, dec: 22.0, description: 'Supernova remnant' },
		},
	},
	andromeda: {
		type: 'galaxy_image',
		...
		notable_objects: {
			'galaxy center': { x: 34768, y: 11115, description: 'Central black hole and bulge of Andromeda' },
			'spiral arms': { x: 25000, y: 8000, description: 'Prominent spiral arm structure' },
			'star forming regions': { x: 40000, y: 13000, description: 'Active star formation areas' },
			'dust lanes': { x: 30000, y: 10000, description: 'Dark dust lanes in the galaxy' },
		},
	},
	mars: {
		...
		bounds: { longitude: { min: -180, max: 180 }, latitude: { min: -90, max: 90 } },
		notable_objects: {
			'olympus mons': { longitude: -133.8, latitude: 18.65, description: 'Largest volcano in the solar system' },
			'valles marineris': { longitude: -75.0, latitude: -14.0, description: 'Largest canyon system on Mars' },
			'hellas basin': { longitude: 70.0, latitude: -42.4, description: 'Largest visible crater on Mars' },
			'polar ice cap': { longitude: 0, latitude: 90, description: 'North polar ice cap' },
			'gale crater': { longitude: 137.8, latitude: -5.4, description: 'Landing site of Curiosity rover' },
			'jezero crater': { longitude: 77.5, latitude: 18.4, description: 'Landing site of Perseverance rover' },
			'chryse planitia': { longitude: -27.0, latitude: 22.5, description: 'Landing site of Viking 1' },
			'utopia planitia': { longitude: 117.5, latitude: 50.0, description: 'Landing site of Viking 2' },
		},
	},
};
...
app.post('/search', async (req, res) => {
	try {
		...
		if (!query || !mapType) return res.status(400).json({ error: 'Query and mapType are required' });
		if (!mapKnowledge[mapType]) return res.status(400).json({ error: `Unsupported map type: ${mapType}` });
...
Return strict JSON: {"found": true|false, "coordinates": {...}, "description": "", "confidence": 0-1, "zoom_level": 2-10}`;
		...
		if (!parsed) return res.status(500).json({ error: 'LLM response was not JSON', raw_response: responseText });
		...
		const normalizedCoords = convertCoordinates(mapType, parsed.coordinates);
		res.json({
			found: true,
...
app.post('/analyze-image', async (req, res) => {
	try {
		...
		if (!imageData || !mapType) return res.status(400).json({ error: 'Image data and map type are required' });
		if (!mapKnowledge[mapType]) return res.status(400).json({ error: `Unsupported map type: ${mapType}` });

		const mapInfo = mapKnowledge[mapType];
		const isDataUrl = typeof imageData === 'string' && imageData.startsWith('data:');
		if (isDataUrl) {
			const base64Len = imageData.replace(/^data:[^;]+;base64,/i, '').length;
			console.log('Received image data (base64 length):', base64Len);
		} else {
			console.log('Received image URL:', String(imageData).slice(0, 128));
		}

		const analysisPrompt = `You are an expert in analyzing ${mapInfo.description}.
Current view (normalized coords): ${currentView ? JSON.stringify(currentView) : 'Unknown'}
...
Analyze the image and return STRICT JSON (no markdown, no code fences):
{
  ...
  "confidence": 0.0-1.0
		...
		if (typeof analysisResult.confidence === 'number' && analysisResult.confidence > 1 && analysisResult.confidence <= 100) {
			analysisResult.confidence = analysisResult.confidence / 100;
		}

		res.json({ success: true, mapType, currentView, analysis: analysisResult, timestamp: new Date().toISOString() });
	} catch (error) {
		console.error('Image analysis error:', error?.response?.data || error);
		res.status(500).json({ error: 'Image analysis failed', message: error.message, detail: error?.response?.data || null });
	}
});
...
app.post('/analyze-viewport', async (req, res) => {
	try {
		...
		if (!viewportData || !mapType) return res.status(400).json({ error: 'Viewport data and map type are required' });
		if (!mapKnowledge[mapType]) return res.status(400).json({ error: `Unsupported map type: ${mapType}` });
		...
		for (const [name, obj] of Object.entries(mapInfo.notable_objects)) {
			let isNearby = false;
			if (mapType === 'unwise') {
				const raDistance = Math.abs(obj.ra - actualCoordinates.centerRA);
				const decDistance = Math.abs(obj.dec - actualCoordinates.centerDec);
				isNearby = raDistance < actualCoordinates.fieldWidth / 2 && decDistance < actualCoordinates.fieldHeight / 2;
				...
				isNearby = xDistance < actualCoordinates.fieldWidth / 2 && yDistance < actualCoordinates.fieldHeight / 2;
			} else if (mapType === 'mars') {
				const lonDistance = Math.abs(obj.longitude - actualCoordinates.centerLon);
				const latDistance = Math.abs(obj.latitude - actualCoordinates.centerLat);
				isNearby = lonDistance < actualCoordinates.fieldWidth / 2 && latDistance < actualCoordinates.fieldHeight / 2;
			}
...
Known objects possibly visible:\n${nearbyObjects.map((o) => `- ${o.name}: ${o.description}`).join('\n') || 'None'}
...
Return STRICT JSON (no markdown):
{
  ...
  "nearby_known_objects": ["${nearbyObjects.map((o) => o.name).join('", "')}"]
		...
		res.status(500).json({ error: 'Viewport analysis failed', message: error.message });
	}
});
...
Known objects:\n${Object.entries(mapInfo.notable_objects)
			...
			.join('\n')}\n`;
		if (imageAnalysis) {
			contextualPrompt += `Current view analysis: ${JSON.stringify(imageAnalysis)}\n`;
		}
		if (currentView) {
			contextualPrompt += `Current viewport (normalized): ${JSON.stringify(currentView)}\n`;
		}
		...
		if (!llmResponse) return res.status(500).json({ error: 'LLM response was not JSON', raw_response: responseText });
		...
		const normalizedCoords = convertCoordinates(mapType, llmResponse.coordinates);
		res.json({
			found: true,
			coordinates: normalizedCoords,
			originalQuery: query,
...
app.listen(PORT, () => {
	...
	console.log(`Tile proxy: http://localhost:${PORT}/tile/unwise/{level}/{x}/{y}.jpg`);
